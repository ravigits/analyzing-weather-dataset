...
def clear(df,col,val):
    ...
    """
    return df[col].value_counts().loc[val]
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    aggregated_value = df.pivot_table(columns=date_col, values=agg_col, aggfunc=agg)
...
def convert(df,celsius):
    ...
    converted_temp['Temp (F)'] =  (converted_temp[celsius] * 9/5)+32
...
weather['Month'] = weather['Date/Time'].apply(lambda x:x.strftime('%Y')) + '-' + weather['Date/Time'].apply(lambda x:x.strftime('%B'))
aggregated = agg_values_ina_month(weather, "Month", "Temp (C)","mean")
aggregated.head()
